                ...
                <div class="flex flex-col gap-4">
                        ...
                        <select name="team1_id" required
                            ...
                            @foreach ($teams as $team)
                                <option value="{{ $team->id }}"
                                    {{ $game->team1_id == $team->id ? 'selected' : '' }}>
                                    {{ $team->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="team2_id" required
                            ...
                            @foreach ($teams as $team)
                                <option value="{{ $team->id }}"
                                    {{ $game->team2_id == $team->id ? 'selected' : '' }}>
                                    {{ $team->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="referee_id"
                            ...
                            @foreach ($referees as $referee)
                                <option value="{{ $referee->id }}"
                                    {{ $game->referee_id == $referee->id ? 'selected' : '' }}>
                                    {{ $referee->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex gap-3 justify-center mt-2">
                        ...
                        <button type="submit" form="delete-form"
                            class="bg-red-400/40 hover:bg-red-500/60 border border-red-400 text-red-800 rounded-md px-4 py-1">
                            Verwijderen
                        </button>
                    </div>
